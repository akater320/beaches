...
#include "beaches/buffered_linestring_aggregator.h"
...
namespace beaches {
	...
	inline void beaches::Graph::load(const char * fileName, std::string outputFileName, const char* gridFilename)
	{
		...
		//exportEdges(fileName, outputFileName);
	...
	inline void beaches::Graph::populateUsedNodes(const char* fileName, bool includeAll) noexcept {
		m_nodeManager.clear();
		...
		osmium::object_id_type prevNode = -1;
		for (const auto& edge : m_edges) {
			if ((edge.Node1() != prevNode) && (includeAll || (edge.hasFlag(EdgeFlags::ReachableFromSea)))) {
				prevNode = edge.Node1();
		...
		for (const auto edgeIndex : m_reversedEdgeIndices) {
			const Edge& edge = m_edges[edgeIndex];
			if ((edge.Node2() != prevNode) && (includeAll || (edge.hasFlag(EdgeFlags::ReachableFromSea)))) {
				prevNode = edge.Node2();
				m_nodeManager.trackNode(prevNode);
			}
		}
	...
	inline void beaches::Graph::exportBufferedLinestrings(const char* osmFile, std::string fileName) noexcept
	{
		...
		populateUsedNodes(osmFile, true); 
	...
	{
		//Load all the locations, even for inland water.
		populateUsedNodes(osmFile, true);
		...
		for (long i = 0; i < m_edges.size(); i++) {
			...
			//if (!(edge.hasFlag(EdgeFlags::CoastOuterRing) || edge.hasFlag(EdgeFlags::ReachableFromSea)))
			//	continue;
			...
			//poFeature->SetField("inner", edge.hasFlag(EdgeFlags::InnerRing));
			//poFeature->SetField("outer", edge.hasFlag(EdgeFlags::OuterRing));
			//poFeature->SetField("erased", edge.hasFlag(EdgeFlags::Erased));
